...
//        if (!['↑','↓'].some(s=>rt.includes(s))) {
//            console.warn(`ルビに↑か↓が一つ必要です。rt:${rt}`)
//        }
    }
    /*
...
//                rbA.kanjis.length < rtA.commas[0].length
                ...
                /*
                    ...
                    ? rbA.kanjis.map((k,i)=>Html.solo(rbA.kanjis[i].text, rtA.commas[0][i], this.#soloPos(pos.first, rtA.has.pipe))).join('')
            ...
            // 字種不問
                ...
                if (1 < rbA.kanjiGroups.length && rtA.has.pipe) { // 漢字群のみルビ ｜論救の御手《ろんきゅう｜みて｜ロジカル・サルベータ》
                    ...
                    /*
                    if (rtA.has.pipes
                    if (rbA.kanjiGroups.length < rtA.pipes.length) {}
                    return (rbA.kanjiGroups.length <= rtA.pipes.length) 
                        ? Html.kanjiGroups(match, rb, rt, rbA, rtA, isUnder)
                        : Html.nest(rb, rtA.pipes[0], rbA, rtA, isUnder)
                    */
...
//                    return Html.solo(rb, rt, pos.first)
                ...
                // ｜ああ漢字いい《かんじ》
                // ｜ああ漢字いい《かんじ｜カンジ》 漢字群＋下全体ルビ or 上下全体ルビ
                // ｜ああ漢字いい《かんじ↓カンジ》
            }
