...
class Comparer {
    ...
    quadMap('ge', i, 
        ...
        ()=>Html.solo(rbs[i], rtLs[i], pos.first), 
        ()=>rbs[i])
    */
    /*
    andE(v) {return [this._L,this._R].every(x=>x===v)}
    orE(v) {return [this._L,this._R].some(x=>x===v)}
    andN(v) {return [this._L,this._R].every(x=>x!==v)}
    orN(v) {return [this._L,this._R].some(x=>x!==v)}
    lev(v)
    llv(v)
    llev(v)
    */
...
class Html {
    ...
    static kanjisTwin(match, rbs, rtFs, rtLs, isUnder=false) { // 親文字全漢字でルビに｜と,があり要素数が同一  ｜山田《やま,だ｜YAMA,DA》
        const pos = {first:isUnder ? 'under' : 'over', last:isUnder ? 'over' : 'under'}
        // 親文字とルビ文字のペア数を算出する
...
//        if (isErr) {console.warn(`${errMsgBase} rb:${btLen.L} rt:${btLen.R} 対象文:${match}\n${btLen.l ? '超過したルビは無視します。' : '不足したルビは親文字をそのまま出力します。'}`)}
        ...
        for (let i=0; i<loopNum; i++) {
            /*
            if (i <= rbs.length) {
                if (i <= rtFs.length && i <= rtLs.length) {htmls.push(Html.nest(rbs[i], rtFs[i], rtLs[i], isUnder))}
                else if (i <= rtFs.length) {htmls.push(Html.solo(rbs[i], rtFs[i], pos.first))}
                else if (i <= rtLs.length) {htmls.push(Html.solo(rbs[i], rtLs[i], pos.first))}
            ...
            */
            if (i <= rbs.length-1) {
                ...
                else if (i <= rtFs.length-1) {htmls.push(Html.solo(rbs[i], rtFs[i], 'over'===pos.first ? '' : 'under'))}
                else if (i <= rtLs.length-1) {htmls.push(Html.solo(rbs[i], rtLs[i], pos.last))}
//                else if (i <= rtFs.length-1) {htmls.push(Html.solo(rbs[i], rtFs[i], 'over'===pos.first ? '' : 'under'))}
//                else if (i <= rtLs.length-1) {htmls.push(Html.solo(rbs[i], rtLs[i], 'over'===pos.first ? '' : 'under'))}
                else {htmls.push(rbs[i])} // ルビより多い親文字はそのまま出力する
            } // 親文字より多いルビは無視する
...
//        const rbA = (rtA.has.commas[0]) ? Analizer.rb(rb) : null
...
//        else if (rtA.has.commas[0]) { return Html.kanjis(match, rbA, rtA) }//漢字一字ずつルビ。任意で下にも。
