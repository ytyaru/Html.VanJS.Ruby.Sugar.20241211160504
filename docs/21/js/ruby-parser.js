...
class Html {
    ...
    */
    ...
    /*
    static pipeKanjis(match, rbA, rtA, isUnder=false) { // 親＆ルビ文字が｜分割され同数＆親文字が全漢字＆ルビ文字に,があり同数
        // ｜は無視して漢字一字ずつにルビを振るだけ
        return rbA.kanjis.map((k,i)=>this.solo(k, rtA.commas[i])).join('')
    }
    */
    ...
    static kanjiPipeCommas(match, rbA, rtA, isUnder) {
        ...
        for (let p=0; p<rbA.pipes.length; p++) {
            ...
            for (let i=L; i<kanjis.length; i++) {htmls.push(kanjis[i])}
        }
        return htmls.join('')
    }
}
...
//        if ('kanjiGroups' in rbD && rtA.has.pipe) { return Html.pipes(rubyText, rbA, rtA, isUnder) }
        const rtA = Analizer.rt(rt)
        //if (rbD.pipes && rtA.has.pipe && rbD.pipes.length===rtA.pipes.length) {return Html.pipes(rubyText, rbD.pipes, rtA, isUnder)} // 親文字に｜がある
...
//        const isKanjiOnly = rb===rbA.kanjis.map(k=>k.text).join('')
        ...
        console.log(rbD.pipes, rtA.has.pipe, rbD.pipes ? rbD.pipes.length : null, rtA.pipes.length, rtA.has.commas, Analizer.isAllKanji(rb.replaceAll('｜','')), rtA)

        if (rbA.has.pipe) {
            if (rtA.has.pipe && rbA.pipes.length===rtA.pipes.length // 親＆ルビ文字に｜があり同数
             && rtA.has.commas.some(c=>c)                           // ルビ文字に,があり
             && Analizer.isAllKanji(rb.replaceAll('｜',''))) {      // 親文字が全部漢字で,と同数
                console.log('**********')
                // ｜は無視して漢字一字ずつにルビを振るだけ
                //const commas = rtA.commas.reduce((all,now,i)=>all.concat(now), [])
                return Html.kanjiPipeCommas(rubyText, rbA, rtA, rbD.isUnder)
                /*
                const commas = rtA.commas.map((comma,i)=>comma.slice(0, Math.min(comma.length, rbA.pipes[i].length)))
                if (rbA.kanjis.length!==commas.length){console.warn(`親文字とルビ文字に｜がありルビ文字に,がある場合、親文字はすべて漢字であり、かつ親文字数とカンマ要素数は同数であるべきです。\n親文字の先頭からルビを振ります。数が違うため対応する字と異なる可能性があります。ルビ不足なら親文字をそのまま出力します。ルビ超過なら無視します。\n対象文字:${rubyText} rb:${rbA.kanjis.length} rt:${commas.length}`)}
                const htmls = []
                for (let p=0; p<rbA.pipes.length; p++) {
                    const kanjis = Array.from(rbA.pipes[p])
                    const L = Math.min(kanjis.length, commas[p].length)
                    for (let i=0; i<L; i++) {htmls.push(Html.solo(kanjis[i], commas[p][i], rbD.isUnder ? 'under' : ''))}
                    for (let i=L; i<kanjis.length; i++) {htmls.push(kanjis[i])}
                }
                return htmls.join('')
                */
                ...
                /*
                const commas = rtA.commas.reduce((all,now,i)=>all.concat(now.slice(0, Math.min(now.length, rbA.pipes[i].length))), [])
                
                if (rbA.kanjis.length!==commas.length){console.warn(`親文字とルビ文字に｜がありルビ文字に,がある場合、親文字はすべて漢字であり、かつ親文字数とカンマ要素数は同数であるべきです。\n親文字の先頭からルビを振ります。数が違うため対応する字と異なる可能性があります。ルビ不足なら親文字をそのまま出力します。ルビ超過なら無視します。\n対象文字:${rubyText} rb:${rbA.kanjis.length} rt:${commas.length}`)}
                const L = Math.min(rbA.kanjis.length, commas.length)
                const htmls = []
                for (let i=0; i<L; i++) {htmls.push(Html.solo(rbA.kanjis[i].text, commas[i], rbD.isUnder ? 'under' : ''))}
                for (let i=L; i<rbA.kanjis.length; i++) {htmls.push(rbA.kanjis[i].text)}
                return htmls.join('')
                //return rbA.kanjis.map((k,i)=>Html.solo(k.text, rtA.commas[i])).join('')
//                return rbA.kanjis.map((k,i)=>Html.solo(k.text, commas[i])).join('')
                */
...
//        newLines[newLines.length-1].index
...
//                    const kanjiGroups = matches.slice(0, i).map(m=>m[0])
                    return {
                        ...
                        start:oldRtEnd + matches[matches.length-1].index, end:newRtStart,
                        isLong:isLong, isShort:isShort, isUnder:isUnder, isEscape:isEscape,
    //                    kanjis:Array.from(kanjiGroups), kanjiGroups:kanjiGroups,
    //                    kanjiGroups:kanjiGroups,
                        pipes: pipes,
