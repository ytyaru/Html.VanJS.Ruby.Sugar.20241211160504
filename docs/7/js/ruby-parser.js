...
//        const rbA = Analizer.rb(rb)
        ...
        //for (let i=0; i<rbA.kanjis.length; i++) {
            //htmls.push(Html.solo(rbA.kanjis[i].text, rtA.commas[0][i], (1 < rtA.pipes.length) ? 'over' : ''))
            //htmls.push(Html.solo(rbA.pipes[i].text, rtA.pipes[0][i], (rbA.pipes.length < rtA.pipes.length) ? 'over' : ''))
...
//        const rbA = Analizer.rb(rb)
    ...
    static kanjiGroups(match, rbA, rtA, isUnder=false) { // 親文字は漢字とそれ以外が混在しておりルビ文字は｜で区切られている
        /*
        const pos = {
            //first: (rbA.kanjis.length < rtA.commas[0].length) ? (isUnder ? 'under' : 'over') : ''),
...
//        const rbA = Analizer.rb(rb)
        //if (rtA.commas[0].length!==rbA.kanjis.length) {
        if (rtA.pipes.length!==rbA.kanjiGroups.length) {
            console.warn(`親文字が漢字とそれ以外混在しており、ルビ文字が｜で区切られている時は、親文字の漢字群とルビ文字の｜群数が同じであるべきです。ルビ数:${rtA.pipes.length} 漢字群数:${rbA.kanjiGroups.length} 対象文:${match}`)
            return match
        }
        const htmls = []
        for (let i=0; i<rbA.kanjiGroups.length; i++) {
//        for (let i=0; i<rbA.kanjis.length; i++) {
//            htmls.push(Html.solo(rbA.kanjis[i].text, rtA.commas[0][i], (1 < rtA.pipes.length) ? 'over' : ''))
            htmls.push(Html.solo(rbA.kanjiGroups[i].text, rtA.pipes[i], (1 < rtA.pipes.length) ? 'over' : ''))
        }
        //if (1 < rtA.pipes.length) { return Html.solo(htmls.join(''), rtA.pipes[rbA.kanjis.length], 'under') }
        if (rbA.kanjiGroups.length < rtA.pipes.length) { return Html.solo(htmls.join(''), rtA.pipes[rbA.kanjis.length], 'under') }
        else {return htmls.join('')}
        */
        const pos = {
            //first: (rbA.kanjis.length < rtA.commas[0].length) ? (isUnder ? 'under' : 'over') : ''),
            ...
            for (let parts of alls) {
                if (parts.isKanji) {
                    if (kanjiCount < kanG.length && kanjiCount < rtA.pipes.length) {
                        ...
                        //htmls.push(Html.solo(kanG[kanjiCount].text, rtA.pipes[kanjiCount], dir.first))
//                        htmls.push(`<ruby style="ruby-position:${dir.first};">${kanG[kanjiCount].text}<rp>（</rp><rt>${rts[kanjiCount]}</rt><rp>）</rp></ruby>`)
            ...
            if (kanjiCount < rtA.pipes.length) { return Html.solo(htmls.join(''), rtA.pipes[kanjiCount], post.last) }
            //if (kanjiCount < rtA.pipes.length) { return Html.solo(htmls.join(''), rtA.pipes[kanjiCount], dir.last) }
            //if (kanjiCount < rtA.pipes.length) { return `<ruby style="ruby-position:${dir.last};">${htmls.join('')}<rp>（</rp><rt>${rtA.pipes[kanjiCount]}</rt><rp>）</rp></ruby>` }
            else {return htmls.join('')}
        } else { return Html.solo(rb, rt, pos.first) }
        //} else { return Html.solo(rb, rt, ('over'===dir.first) ? '') }
        //} else { return `<ruby${('over'===dir.first) ? '' : ' style="ruby-position:under;"'}>${rb}<rp>（</rp><rt>${rt}</rt><rp>）</rp></ruby>` }
    }

    /*
    static toShort(match, rb, rt) { // 親文字が漢字だけ（パイプもない）
        ...
        if (rtA.has.commas[0]) { // 漢字一字ずつルビを振る。任意で下にもルビを振る。
            ...
            const htmls = []
            for (let i=0; i<rbA.kanjis.length; i++) {
                htmls.push(this.solo(rbA.kanjis[i].text, rtA.commas[0][i], (1 < rtA.pipes.length) ? 'over' : ''))
            }
            if (1 < rtA.pipes.length) { return this.solo(htmls.join(''), rtA.pipes[rbA.kanjis.length], 'under') }
            else {return htmls.join('')}
        }
        else if (rtA.has.pipe) { return Html.nest(rb, rtA.pipes[0], rtA.pipes[1]) } // 上下に一つずつルビを振る
        else { return this.solo(rb, rt) } // 上に一つルビを振る
    }
    */
}
class Analizer {
...
class LongRubyParser {
    static #LONG = /([｜↓])([^《》\n\r]{1,50})《([^《》\n\r]{1,100})》/g
    static #soloPos(pos) { return 'over' ? '' : pos }
    static #toHtml(match, D, rb, rt) {
        ...
        if (rbA.has.kanji) {
            ...
            if (rb===rbA.kanjis.map(k=>k.text).join('')) {
                ...
                // 一字ルビ
                /*
                if (!rbA.has.pipe && !rtA.has.pipe) {
                    if (rtA.commas.some(c=>c)) { return Html.commas(match, rbA, rtA, isUnder) } // 一字ルビ
//                        return Html.solo(rtA.commas.map((c,i)=>Html.solo(rbA.kanjis[i], rtA.commas[0][i]), pos).join(''), )
                    else { return Html.solo(rb, rt, pos) } // 全体ルビ
                } else { return Html.pipes(match, rbA, rtA, isUnder) } // 部分ルビ
//                        Html.solo(Html.commas(match, rbA, rtA, isUnder), rtA.commas[1]
//                }
                */
            ...
            // 漢字を含む
                ...
                if (1 < rbA.kanjiGroups.length) { // 漢字群のみルビ     ｜論救の御手《ろんきゅう｜みて｜ロジカル・サルベータ》
                    return Html.kanjiGroups(match, rbA, rtA, isUnder)
                } else { // 全体ルビ         ｜あ山《よみ｜ヨミ》
